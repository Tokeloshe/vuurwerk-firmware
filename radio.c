...
//				BK4819_SetFilterBandwidth(Bandwidth, gRxVfo->Modulation == MODULATION_AM && gSetting_AM_fix);
	...
	// Restore MIC AGC after possible AM/USB TX
	BK4819_WriteRegister(BK4819_REG_19, 0x1041);

	// Reset STE on any radio reconfiguration
...
//				BK4819_SetFilterBandwidth(Bandwidth, gCurrentVfo->Modulation == MODULATION_AM && gSetting_AM_fix);
	...
	// === VUURWERK v1.2.3 AM/USB TX intelligence ===
	// MIC AGC off: stops gain pumping that causes sporadic AM/USB sound
	// Deviation reduced: narrows occupied BW, cleaner on AM/SSB receivers
	if (gCurrentVfo->Modulation != MODULATION_FM) {
		BK4819_WriteRegister(BK4819_REG_19, 0x9041);
		BK4819_WriteRegister(BK4819_REG_50,
			(gCurrentVfo->Modulation == MODULATION_AM) ? 0x180D : 0x0F08);
	}

	// === VUURWERK v1.2.3 spurious reduction ===
	// PA gain staged for frequency, bias held at 0.
	// TX_SOFT_START ramps bias from 0 to target cleanly.
	// Eliminates 10ms full-power burst on PTT.
	BK4819_SetupPowerAmplifier(0, gCurrentVfo->pTX->Frequency);

	SYSTEM_DelayMs(5);

	// External PA enabled LAST: only ever sees the soft-start ramp
