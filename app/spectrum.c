...
// Spectrum display modes: 0=NORMAL, 1=PEAK, 2=MTI, 3=VOICE
static uint8_t peak_hold[128];
static uint8_t prev_sweep[128];
static uint8_t spectrum_mode = 0;
...
// Voice probability from single-shot RSSI + noise indicator
static uint8_t ComputeVoiceProb(uint16_t rssi, uint8_t noise)
{
  if (rssi < 50 || noise > 200) return 0;

  int16_t s = 0;
  if      (noise < 30) s += 45;
  else if (noise < 70) s += 20;
  if      (rssi > 100) s += 35;
  else if (rssi > 65)  s += 15;
  if (rssi > 80 && noise < 40) s += 15;
  return (s > 95) ? 95 : (uint8_t)s;
}
...
static uint32_t scanFStart;  // cached for VoiceHop

static void InitScan() {
  ResetScanStats();
  scanInfo.i = 0;
  scanFStart = scanInfo.f = GetFStart();
...
static void Measure()
{
  ...
  // Update spectrum mode buffers
  if (scanInfo.i < 128) {
    uint8_t val = Rssi2PX(rssi, 0, 55);
    prev_sweep[scanInfo.i] = val;
    if (val > peak_hold[scanInfo.i])
      peak_hold[scanInfo.i] = val;

    // VOIC mode: capture noise indicator (already settled from RSSI delay)
    if (spectrum_mode == 3)
      noiseHistory[scanInfo.i] = BK4819_ReadRegister(0x65) & 0xFF;
  }
}
...
static void DrawSpectrum() {
  ...
  default:
  case 0: // NORMAL + PEAK HOLD (mode 1 adds peak dots)
  case 1:
    for (uint8_t x = 0; x < 128; ++x) {
      ...
      if (spectrum_mode == 1) {
        ...
        if (peak_hold[bin] > 0) {
          uint8_t py = DrawingEndY - peak_hold[bin];
          if (py < DrawingEndY)
            PutPixel(x, py, true);
        }
      }
  ...
  case 2: // MTI (Moving Target Indicator): XOR of current vs previous
    for (uint8_t x = 0; x < 128; ++x) {
      uint8_t bin = x >> settings.stepsCount;
      uint8_t cur = Rssi2PX(rssiHistory[bin], 0, DrawingEndY);
      uint8_t diff = prev_sweep[bin] ^ cur;
      if (diff > 0) {
        uint8_t y = DrawingEndY - (diff > DrawingEndY ? DrawingEndY : diff);
        DrawVLine(y, DrawingEndY, x, true);
      }
    }
    break;
  ...
  case 3: // VOICE mode — show bars only where voice detected
    for (uint8_t x = 0; x < 128; ++x) {
      ...
      uint16_t rssi = rssiHistory[bin];
      if (rssi == RSSI_MAX_VALUE) continue;
      uint8_t h = Rssi2PX(rssi, 0, DrawingEndY);
      if (vp < 75) h >>= 1;
...
static const char *specModeNames[] = {"NORM", "PEAK", " MTI", "VOX "};
...
static void RenderStill() {
  ...
  for (int i = 0, idx = 1; idx <= 4; ++i, ++idx) {
    ...
    GUI_DisplaySmallest(registerSpecs[idx].name, offset + 2, row * 8 + 2,
                        false, menuState != idx);
...
static void UpdateScan() {
  ...
  // Peak decay: every 10 sweeps, decrement peak_hold
  if (++sweep_count >= 10) {
    sweep_count = 0;
    for (int i = 0; i < 128; i++) {
      if (peak_hold[i]) peak_hold[i]--;
    }
  }

  // VOIC mode: compute voice probability per bin after sweep completes
  if (spectrum_mode == 3) {
    uint16_t steps = GetStepsCount();
    for (uint16_t i = 0; i < steps && i < 128; i++)
      voiceProb[i] = ComputeVoiceProb(rssiHistory[i], noiseHistory[i]);
  }
  ...
  UpdatePeakInfo();
  if (IsPeakOverLevel()) {
    // VOIC mode: skip non-voice signals
    if (spectrum_mode == 3 && !monitorMode && voiceProb[peak.i] < 50) {
      newScanStart = true;
      return;
    }
    ToggleRX(true);
...
static void UpdateListening() {
  ...
  if (spectrum_mode == 3)
    noiseHistory[peak.i] = BK4819_ReadRegister(0x65) & 0xFF;

  if (IsPeakOverLevel() || monitorMode) {
    // VOIC mode: stop listening when noise register says no voice
    if (spectrum_mode != 3 || noiseHistory[peak.i] < 70) {
      listenT = 1000;
      return;
    }
  }
