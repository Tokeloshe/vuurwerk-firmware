...
{
	// === VUURWERK v1.0.9.6 TX CLEANUP (before stock teardown) ===
	TX_COMPRESSOR_Stop();    // restores saved REG_7D
	CTCSS_LEAD_Stop();       // ensures REG_50 unmuted

	// === STOCK EGZUMER (unchanged) ===
...
//	if (gFmRadioCountdown_500ms > 0)
	...
	if (gPowerSaveCountdownExpired && gCurrentFunction == FUNCTION_POWER_SAVE
		...
		else if ((gEeprom.DUAL_WATCH == DUAL_WATCH_OFF && !SCANWATCH_IsActive()) || gScanStateDir != SCAN_OFF || gCssBackgroundScan || goToSleep)
...
void APP_TimeSlice10ms(void)
{
	...
	// === VUURWERK v1.0.9.6 TX PROCESSING (10ms tick, only during TX) ===
	if (gCurrentFunction == FUNCTION_TRANSMIT) {
		CTCSS_LEAD_Process();
	...
	// === VUURWERK v1.0.9.6 RX/IDLE PROCESSING ===
	// Bandscope: sample RSSI every ~100ms (reads REG_67 only)
	BANDSCOPE_Process();

	// Gain staging: self-contained 10ms tick (handles own state checking)
	GAIN_STAGING_10ms(gEeprom.RX_VFO);

	// VFO Split: state machine runs during FUNCTION_FOREGROUND only
	VFO_SPLIT_Process();

	// Scan+Watch: update dwell/hold timer when on watch VFO
	if (SCANWATCH_IsOnWatchVFO())
	{
		bool signal = (gCurrentFunction == FUNCTION_RECEIVE ||
		               gCurrentFunction == FUNCTION_INCOMING);

		if (SCANWATCH_Update(signal))
		{
			// Return to scan VFO, resume scanning
			gEeprom.RX_VFO = gScanWatch.scan_vfo;
			gRxVfo         = &gEeprom.VfoInfo[gEeprom.RX_VFO];
			RADIO_SetupRegisters(false);
			gUpdateDisplay = true;
		}
	...
	// === VUURWERK v1.1.0 Squelch Tail Elimination (reads REG_0C, writes REG_47) ===
	SQUELCH_TAIL_Process();

	// === VUURWERK v1.1.0 Smart Squelch (reads REG_67/65/63, writes REG_78) ===
	...
	// === VUURWERK v1.1.0 RX PROCESSING (software-only, no BK4819 register writes) ===
	{
		...
		if (is_receiving) {
			...
			int16_t rssi_raw = (int16_t)BK4819_GetRSSI();
