...
#endif
		+ ((gRxVfo->Modulation == MODULATION_FM) ? GAIN_STAGING_GetGainDiff(gEeprom.RX_VFO) : 0)
...
void UI_DisplayMain(void)
{
...
#endif
		if (BANDSCOPE_IsEnabled() &&
		    (gCurrentFunction == FUNCTION_FOREGROUND ||
		     gCurrentFunction == FUNCTION_RECEIVE ||
		     gCurrentFunction == FUNCTION_INCOMING))
		{
			center_line = CENTER_LINE_BANDSCOPE;
			memset(gFrameBuffer[3], 0, LCD_WIDTH);
			BANDSCOPE_Render(gFrameBuffer[3]);
		}
		else
	...
	// Context status line on line 6
	{
		char ctx[22];
		ctx[0] = '\0';

		if (gCurrentFunction == FUNCTION_TRANSMIT) {
			const uint16_t elapsed = gEeprom.TX_TIMEOUT_TIMER ?
				(gEeprom.TX_TIMEOUT_TIMER * 120 - gTxTimerCountdown_500ms) : 0;
			const uint16_t secs = elapsed / 2;
			const char *pwr = "L";
			if (gRxVfo->OUTPUT_POWER == OUTPUT_POWER_MID)  pwr = "M";
			if (gRxVfo->OUTPUT_POWER == OUTPUT_POWER_HIGH) pwr = "H";
			sprintf(ctx, "TX %u:%02u PWR:%s", secs / 60, secs % 60, pwr);
		} else if (gCurrentFunction == FUNCTION_RECEIVE || gCurrentFunction == FUNCTION_INCOMING) {
			int16_t rssi_dbm = BK4819_GetRSSI_dBm();
#ifdef ENABLE_AM_FIX
			if (gSetting_AM_fix && gRxVfo->Modulation == MODULATION_AM)
				rssi_dbm += AM_fix_get_gain_diff();
#endif
			if (gRxVfo->Modulation == MODULATION_FM)
				rssi_dbm += GAIN_STAGING_GetGainDiff(gEeprom.RX_VFO);
			int s_level = (rssi_dbm + 127) / 6;
			if (s_level < 0) s_level = 0;
			if (s_level > 9) s_level = 9;
			SignalQuality_t q = SIGNAL_QUALITY_Get();
			...
			uint16_t step = gRxVfo->StepFrequency;
			if (step >= 100)
				sprintf(ctx, "%u.%ukHz %s %s", step / 100, (step % 100) / 10, bw, mod);
			else
				sprintf(ctx, "0.%02ukHz %s %s", step, bw, mod);
		}

		if (ctx[0])
			UI_PrintStringSmallNormal(ctx, 0, 0, 6);
	}
